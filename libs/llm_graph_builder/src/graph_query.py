...
def extract_relationships(records):
    ...
    try:
        ...
        logging.error("graph_query module: An error occurred while extracting relationships from records",
                      exc_info=True)
...
def get_graph_results(uri, username, password, database, document_names):
    ...
    try:
        ...
        raise Exception(
            f"graph_query module: An error occurred in get_graph_results. Please check the logs for more details.") from e
    finally:
