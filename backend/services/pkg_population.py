...
from libs.llm_graph_builder.functions import (upload_file,
...
from backend.services.constants import (QUERY_TO_MERGE_KGs,
...
async def llm_kg_builder_delete(fileName, database):
    try:
        ...
        raise Exception(f"Error while deleting User connections for {fileName}: {str(e)}")
...
async def llm_kg_builder_extract(fileName, database, content, additional_instructions=None, embedding_model=None,
...
async def llm_kg_builder_postprocess(database, embedding_model, embedding_dimension):
    ...
    res = await post_processing(uri, userName, password, database, embedding_model, embedding_dimension)

    logging.info("Creating .parquet files for GraphRAG...")
    initialize_graphRag.create_parquet(uri, userName, password, database, user_dir=user_dir)
...
async def insertion_pipeline(fileName, database, content, additional_instructions=None, embedding_model=None,
                             embedding_dimension=None):
    ...
    if res["status"] == "Success":
        ...
        res = await llm_kg_builder_extract(fileName, database, content, additional_instructions, embedding_model,
                                           embedding_dimension)
        if res["status"] == "Success":
        ...
        logging.info(f"deleting created entities for {fileName} due to uploading failure...")
        await llm_kg_builder_delete(fileName, database)
        return res


async def update_pipeline(fileName, database, content, additional_instructions=None, embedding_model=None,
                          embedding_dimension=None):
    ...
    if res["status"] == "Success":
        ...
        logging.info(f"ATTENTION! Entities for file {fileName} have been deleted and not re-created!")
        return res
...
async def extract_kg_triples(
    ...
    if operation == "insert":
        ...
        res = await insertion_pipeline(fileName, database, json_content, embedding_model=embedding_model,
                                       embedding_dimension=embedding_dimension)
    elif operation == "update":
        logging.info(f"Starting metadata UPDATE for {fileName}...")
        res = await update_pipeline(fileName, database, json_content, embedding_model=embedding_model,
                                    embedding_dimension=embedding_dimension)
    else:
    ...
    # UNSTRUCTURED kind of contents
        ...
        if ext_u == ".txt":
            ...
            logging.info(f"Starting KG extraction for the content of the note {fileName}...")
            r = await insertion_pipeline(fileName_u, database, unstruct_content, embedding_model=embedding_model,
                                         embedding_dimension=embedding_dimension)
            if r["status"] == "Success":
                os.remove(unstruct_path)
            else:
                raise Exception(f"Error while extracting KG from unstructured content of the note {fileName}")
        ...
        # PHOTOS
            ...
            logging.info(f"Starting KG extraction for the content of the image {fileName_u}...")

            r = await insertion_pipeline(fileName_u, database, img_description, embedding_model=embedding_model,
                                         embedding_dimension=embedding_dimension)
            if r["status"] == "Success":
                os.remove(new_unstruct_path)
            else:
                os.remove(new_unstruct_path)
                raise Exception(f"Error while extracting KG from unstructured content of the image {fileName_u}")
        ...
        # DOCs
            ...
            logging.info(f"Starting KG extraction for the content of the document {fileName_u}...")
            r = await insertion_pipeline(fileName_u, database, unstruct_content, embedding_model=embedding_model,
                                         embedding_dimension=embedding_dimension)
            if r["status"] == "Success":
                os.remove(unstruct_path)
            else:
                raise Exception(f"Error while extracting KG from unstructured content of the document {fileName_u}")
        ...
        try:
            ...
            raise Exception(f"Error while merging structured and unstructured sub-KGs for {fileName}: {str(e)}")
