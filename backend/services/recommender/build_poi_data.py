...
from backend.services.recommender.io_utils import load_parquet
...
def build_poi_data_pipeline():
    ...
    if not all(os.path.exists(f) for f in raw_input_files):
        logging.error("Raw data files not found in 'data/recommender/raw/'. Cannot build POI data.")
        logging.error("Please download the TIST2015 dataset and place the files in the correct folder.")
    ...
    venues_to_fetch_df.to_parquet(os.path.join(processed_path, "venues_to_fetch.parquet"))
