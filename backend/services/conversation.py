...
def get_llm(model_env_value):
    if model_env_value.startswith("groq"):
        ...
        llm = ChatOpenAI(
            api_key=api_key, base_url=api_endpoint, model=model_name, temperature=0
        )
    ...
    def __init__(self, database: str, embedding_model, user_id: int):
        ...
        async def img_deep_analysis(question: str) -> str:
            ...
            try:
                ...
                photo_ids = [
                    line.strip()[2:]
                    for line in photo_list_str.strip().splitlines()
                    if line.strip().startswith("- ")
                ]
                # analyzes the top similar photos (given the query) one by one and returns possible responses
                final_response = await img_analyzer(
                    question=question, photo_ids=photo_ids, user_id=self.user_id
                )
                ...
                logger.info(
                    f"Error in img_deep_analysis for user {self.user_id}: {e}",
                    exc_info=True,
                )
        ...
        async def graph_RAG(question: str) -> str:
            ...
            try:
                ...
                logger.info(
                    f"Error in graph_RAG for user {self.user_id}: {e}", exc_info=True
                )
...
class ReactAgent:
    ...
    async def evaluate_rag_response(self, state: AgentState) -> AgentState:
        ...
        if decision not in ["bad", "good"]:
            raise Exception(
                f"Invalid response from evaluate_rag_response node: {decision}"
            )
    ...
    def create_agent(self, checkpointer):
        ...
        graph.add_conditional_edges(
            ...
            {"evaluate_rag": "evaluate_rag", "standard_agent": "standard_agent"},
        )

        graph.add_conditional_edges(
            "evaluate_rag",
            self.router_after_evaluation,
            {"fallback_agent": "fallback_agent", "standard_agent": "standard_agent"},
        )
...
async def process_user_command(
    user_id: int, db_name: str, command_text: str, embedding_model
) -> ChatResponse:
    """
    ...
    try:
        ...
        assistant_response = await graph_agent.ainvoke(
            AgentState(
                messages=[HumanMessage(role="user", content=command_text)],
                rag_evaluation=None,
            ),
            config=config,
        )
        ...
        logger.error(
            f"Error processing user command for user_id={user_id}: {e}", exc_info=True
        )
        return ChatResponse(
